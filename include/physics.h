...
glm::vec3 gravity(0.0f, -0.01f, 0.0f); // Gravity std::vector
...
enum class state_collision {
    NONE, // No collision detected
    MIGHT_COLLIDE, // Potential collision detected
    COLLISION_WITH_OBJECT // Collision with another object
};
...
class AABB {
    public:
        ...
        state_collision state = state_collision::NONE; // State of the AABB for collision detection

        // Number of faces contained within this AABB and a vector to store them
        long long num_faces = 0;
        std::vector<face*> faces;
        
        object* showing_tree = nullptr;
        ...
        AABB() : obj(nullptr), min(glm::vec3(0.0f)), max(glm::vec3(0.0f)) {
            ...
            state = state_collision::NONE; // Initialize state to NONE
            num_faces = 0;
            isLeaf = true;
        ...
        void setup_tree(unsigned int i){
            obj->m->setup_mesh(i); // Set up the mesh for the object
            showing_tree->m->setup_mesh(i); // Set up the mesh for the showing tree
        }
        ...
        void detect_collision(AABB* other, std::vector<face*>& collision_faces) {
            ...
            if (num_faces <= 20 and other->num_faces <= 20) {
                ...
                for (const auto& f : faces) {
                    for (const auto& of : other->faces) {
                        ...
                        if(triangleTriangleIntersectionComplete(tr1, tr2)){
                            other->state = state_collision::COLLISION_WITH_OBJECT; // Set collision state
                            state = state_collision::COLLISION_WITH_OBJECT; // Set collision state
                            std::cout << "Collision detected between faces with vertices: " 
                                      << f->vertices[0] << ", " << f->vertices[1] << ", " << f->vertices[2]
                                      << " and " << of->vertices[0] << ", " << of->vertices[1] << ", " << of->vertices[2] 
                                      << std::endl;
                ...
                other->state = state_collision::MIGHT_COLLIDE; // Set potential collision state
                state = state_collision::MIGHT_COLLIDE; // Set potential collision state
        ...
        void update(){
            ...
            calculate_bounds();
            set_showing_positions(); // Update the showing tree positions based on the new bounds
            reset_state(); // Reset the state of the AABB after updating positions
        }

        void draw_children(Shader shader) {
            if (!showing_tree) return;

            switch (state)
            {
            case state_collision::NONE:
                // Set the color to green for no collision
                showing_tree->m->mod->mat.ambient[0] = 0.0f;
                showing_tree->m->mod->mat.ambient[1] = 1.0f;
                showing_tree->m->mod->mat.ambient[2] = 0.0f;
                break;
            
            case state_collision::MIGHT_COLLIDE:
                // Set the color to yellow for potential collision
                showing_tree->m->mod->mat.ambient[0] = 1.0f;
                showing_tree->m->mod->mat.ambient[1] = 1.0f;
                showing_tree->m->mod->mat.ambient[2] = 0.0f;
                break;
                
            case state_collision::COLLISION_WITH_OBJECT:
                // Set the color to red for collision with an object
                showing_tree->m->mod->mat.ambient[0] = 1.0f;
                showing_tree->m->mod->mat.ambient[1] = 0.0f;
                showing_tree->m->mod->mat.ambient[2] = 0.0f;
                break;
            }

            showing_tree->draw(shader); // Draw the showing tree for visualization

            // Draw the children AABBs if they exist
            for (int i = 0; i < 2; ++i) {
                if (children[i]) {
                    children[i]->draw_children(shader);
                }
            }
        }
        ...
        void draw(Shader shader) {
            if (showing_tree) {
                ...
                draw_children(shader); // Draw the showing tree for visualization
                glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); // Switch back to fill mode
            }
            obj->draw(shader); // Draw the object
        }
        
        void reset_state() {
            state = state_collision::NONE; // Reset the state of the AABB
            for (int i = 0; i < 2; ++i) {
                if (children[i]) {
                    children[i]->reset_state(); // Recursively reset the state of child AABBs
                }
            }
        }
    ...
    private:
        ...
        void set_showing_positions(){
            if (!showing_tree) return;

            calculate_bounds(); // Ensure bounds are calculated before setting positions

            // Set the positions of the showing tree based on the AABB's min and max bounds
            showing_tree->position.clear();
            showing_tree->position.push_back(glm::vec3(max.x, max.y, min.z));
            showing_tree->position.push_back(glm::vec3(max.x, min.y, min.z));
            showing_tree->position.push_back(max);
            showing_tree->position.push_back(glm::vec3(max.x, min.y, max.z));
            showing_tree->position.push_back(glm::vec3(min.x, max.y, min.z));
            showing_tree->position.push_back(min);
            showing_tree->position.push_back(glm::vec3(min.x, max.y, max.z));
            showing_tree->position.push_back(glm::vec3(min.x, min.y, max.z));
            
            for (int i = 0; i < 2; i++) {
                if (children[i]) {
                    children[i]->set_showing_positions(); // Recursively set positions for children
                }
            }

        }

        // Function to set up the showing tree for visualization
        void setup_showing_tree() {
            if (!obj) return;
            showing_tree = new object("../models/aabb_box.obj");
            //std::cout << "Showing tree created." << std::endl;
            set_showing_positions(); // Set the positions of the showing tree based on the trees min and max bounds
            showing_tree->isStatic = true; // Set the showing tree as static
        ...
        void generate_children() {
            ...
            children[0]->setup_showing_tree();
            children[0]->showing_tree->m->setup_mesh(0); // Set up the mesh for the first child
            ...
            children[1]->setup_showing_tree();
            children[1]->showing_tree->m->setup_mesh(0); // Set up the mesh for the second child
...
class world {
    public:
        ...
        ~world() {
            for (int i = 0; i < num_tr; i++) {
                ...
                if (tr) {
                    ...
                    delete tr->showing_tree; // Clean up the showing tree
        ...
        void start_world() {
            for (int i = 0; i < num_tr; i++) {
                trees[i]->setup_tree(i); // Set the positions of each tree
            }
        }
